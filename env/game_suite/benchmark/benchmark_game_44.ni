...
The washroom part 0 is some text that varies. The washroom part 0 is "You find yourself in a washroom. A normal kind of place. You start to take note of what's in the room.
 ...
 You see a gleam over in a corner, where you can see a rack. The rack is usual.[if there is something on the s_0] On the rack you can make out [a list of things on the s_0].[end if]".
...
The cookhouse part 0 is some text that varies. The cookhouse part 0 is "Guess what, you are in a place we're calling a cookhouse. You try to gain information on your surroundings by using a technique you call 'looking.'

 You can see [if c_1 is locked]a locked[else if c_1 is open]an opened[otherwise]a closed[end if]".
The cookhouse part 1 is some text that varies. The cookhouse part 1 is " fridge.[if c_1 is open and there is something in the c_1] The fridge contains [a list of things in the c_1].[end if]".
...
The description of o_0 is "The backup calendar would seem to be well matched to everything else here".
...
Test quest0_0 with "take key from rack / go east / unlock safe with key / open safe / insert backup calendar into safe"
...
The objective part 0 is some text that varies. The objective part 0 is "You are now playing a profound round of TextWorld! Here is how to play! First, it would be good if you could pick up the key from the rack. After getting the key, make an effort to head east. And then".
The objective part 1 is some text that varies. The objective part 1 is ", unlock the safe inside the cubicle. And then, assure that the safe within the cubicle is wide open. After opening the safe, insert the backup calendar into the safe inside the cubicle. And once you'".
